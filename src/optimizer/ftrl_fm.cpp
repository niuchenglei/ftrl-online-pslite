...
void FtrlFM::set_param (float alpha, float beta, float l1, float l2) { //float alpha,beta,l1,l2) {
    this->alpha = alpha;
    this->beta = beta;
    this->lamda1 = l1;
    this->lamda2 = l2;
...
void FtrlFM::update(KVPairs<float> &res, const KVPairs<float> &req) {
    time_t t = time(0);
    char tmp[64];
    ...
    for (size_t i=0; i<res.keys.size(); ++i) {
        ...
        // update V, NV, ZV
        for (int k=0; k<K; k++) {
            float Ni = NV[key][k] + g*g;
            Z[key][k] += g - (std::sqrt(Ni)-std::sqrt(N[key][k]))/alpha*V[key][k];
            N[key][k] = Ni;
            if (fabs(Z[key][k]) <= lamda1) {
                V[key][k] = 0;
            }
            else {
                float sign = (Z[key][k] > 0.0)?1:-1;
                float tmpr = Z[key][k] - sign*lamda1;
                float tmpl = -((beta+std::sqrt(N[key][k]))/alpha+lamda2);
                V[key][k] = tmpr/tmpl;
            }
        }
    }
...
void FtrlFM::set_paramfromkv(KVPairs<float> &res, const KVPairs<float>&req) {
...
void FtrlFM::set_weight(KVPairs<float>& res, const KVPairs<float>& req) {
    // printf("set weight\n");
    for (size_t i=0; i<res.keys.size(); ++i) {
        uint64_t key = res.keys[i];
        float v = req.vals[i];
...
void FtrlFM::set_v(KVPairs<float>& res, const KVPairs<float>& req) {
    // not implement
    /*for (size_t i=0; i<res.keys.size(); ++i) {
        uint64_t key = res.keys[i];
        float v = req.vals[i];
...
void FtrlFM::handle_request(const KVMeta& req_meta, const KVPairs<float>& req_data, KVServer<float>* server) {
